...
from tpcbench import postgresqldb as pgdb, result as r, load
...
def refresh_func1(conns, routes, data_dir, update_dir, stream, num_streams, verbose):
...
def refresh_func2(conns, routes, data_dir, delete_dir, stream, num_streams, verbose):
...
def run_query_stream(conns, routes, query_root, generated_query_dir, stream, num_streams, result, verbose):
    """
    ...
    for i in range(0, 22):
        try:
            ...
            result.startTimer()
            conns[routes[order[i] - 1]].executeQueryFromFile(filepath)
...
def run_power_test(query_root, data_dir, update_dir, delete_dir, generated_query_dir, results_dir,
    ...
    try:
        ...
        #
        if not read_only:
            if refresh_func1(conns, routes, data_dir, update_dir, stream, num_streams, verbose):
                return 1
        result.setMetric(REFRESH_METRIC % (stream, 1), result.stopTimer())
        #
        if run_query_stream(conns, routes, query_root, generated_query_dir, stream, num_streams, result, verbose):
            return 1
        #
        result.startTimer()
        if not read_only:
            if refresh_func2(conns, routes, data_dir, delete_dir, stream, num_streams, verbose):
                return 1
...
def run_throughput_test(query_root, data_dir, update_dir, delete_dir, generated_query_dir, results_dir,
    ...
    try:
        ...
        for i in range(num_streams):
            stream = i + 1
            print(f'refresh fn: stream {stream} of {num_streams}...')
            # refresh functions
            result.startTimer()
            if not read_only:
                if refresh_func1(conns, routes, data_dir, update_dir, stream, num_streams, verbose):
...
def do_load(conns, query_root, data_dir):
    database = 'tpchdb'
    for i, conn in enumerate(conns):
        if load.clean_database(conn, TABLES):
            print(i, "could not clean the database.")
            exit(1)
        print(i, "cleaned database %s" % database)
        if load.create_schema(query_root, conn, database, PREP_QUERY_DIR):
            print(i, "could not create schema.")
            exit(1)
        print(i, "done creating schemas")
        if load.load_tables(data_dir, conn, database, TABLES, LOAD_DIR):
            print(i, "could not load data to tables")
            exit(1)
        print(i, "done loading data to tables")
        if load.index_tables(query_root, conn, database, PREP_QUERY_DIR):
            print(i, "could not create indexes for tables")
            exit(1)
...
def main(replicas: list[Replica], routes: list[int], index_config,
    ...
    data_dir = os.path.join('.', 'tpcbench', 'data')

    print('cleaning and reloading databases....')
    do_load(conns, query_root, data_dir)
    print('cleaned and reloaded databases!')

    print('constructing indexes...')
    for i, conn in enumerate(conns):
        for num, tbl in enumerate(index_config[i]):
            conn.executeQuery('CREATE INDEX candidate_index_%d ON %s (%s);' % (num, tbl[0], ', '.join(tbl[1])))
        conn.commit()
    print('constructed indexes!')

    if run_power_test(query_root, data_dir, UPDATE_DIR, DELETE_DIR, GENERATED_QUERY_DIR, RESULTS_DIR,
                            conns, routes,
                            run_timestamp, num_streams, verbose, read_only):
        print("running power tests failed")
        exit(1)
    # Throughput tests
    if run_throughput_test(query_root, data_dir, UPDATE_DIR, DELETE_DIR, GENERATED_QUERY_DIR, RESULTS_DIR,
                                    replicas, conns, routes,
                                    run_timestamp, num_streams, verbose, read_only):
