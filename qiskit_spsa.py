...
This implementation is the same as found in `qiskit_algorithms.optimizers.SPSA`,
but removes the second-order (Hessian) estimations due to memory considerations.

The second-order estimates were not used in the original qDINA implementation,
but vestigial parts required huge memory allocations that made the virtual machines
very, very unhappy. As a happy coincidence, this may slightly reduce the recommendation
time.
"""
