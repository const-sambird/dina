...
class IndexSelectionEnv(gym.Env):
    def __init__(self, profiler: Profiler, replicas: list[Replica], router: Router, candidates,
        ...
        self._potential_loop_state = None
    ...
    def step(self, action: int):
        ...
        if (self._state == self._potential_loop_state).all():
            print('loop penalty:       5.0')
            reward -= 5
        else:
            print('loop penalty:       0.0')
        self._potential_loop_state = self._previous_state.copy() if self._previous_state is not None else None
