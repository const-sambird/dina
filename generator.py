...
class WorkloadGenerator:
    def __init__(self, qgen_path: str, template_path: str, queries_per_template: int | list[int], workload_path: str, scale_factor: int):
        '''
        ...
        self.qgen_path = os.path.normpath(os.path.join(rootdir, qgen_path))
        self.template_path = os.path.normpath(os.path.join(rootdir, template_path))
        ...
        if isinstance(queries_per_template, list):
            ...
            self.queries_per_template = [queries_per_template for _ in range(22)]
        ...
        for i in range(22):
            for j in range(self.queries_per_template[i]):
                ...
                with open(f'{self.workload_path}/{i + 1}_{j}.sql', 'w') as outfile:
                    subprocess.run([f'{self.qgen_path}/qgen', '-s', self.scale_factor, '-r', str(SEED_BASE + j), str(i + 1)],
                                cwd=self.qgen_path,
